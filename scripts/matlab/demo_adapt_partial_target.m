function demo_adapt_partial_target()
  rng(0);
  tf = 100;
  % -------- Fixed (chaser) --------
  ...
  Gamma = diag([0.3,0.3,0.3]);      % much larger than before
  ...
  title('Plot the TARGET part, not total');

  figure('Name','Total inertia diagonal (will change but less)');
