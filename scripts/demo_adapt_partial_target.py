...
def dyn_partial_adapt_LQR(t, x, J_fixed, S, J_true, K, Gamma):
    """
    Dynamics for partial inertia adaptation with LQR.
    ...
    S       : (3,3,3) basis matrices S_i for target inertia
    J_true  : (3,3) full true post-dock inertia
    ...
    for i in range(3):
        Jhat += alpha[i] * S[:, :, i]
    Jhat = 0.5 * (Jhat + Jhat.T)  # enforce symmetry

    # Small-angle attitude error: delta_theta ≈ 2*q_vec
    eR = 2.0 * q[1:4]               # (3,)
    x_e = np.concatenate([eR, w])   # (6,)

    # LQR control
    u_LQR = -K @ x_e                # (3,)

    # Include gyroscopic term with Jhat so Jhat affects torque
    Jhatw = Jhat @ w
    u = u_LQR + np.cross(w, Jhatw)  # (3,)
    ...
    # Regressor columns: Y_i = -(S_i * wdot), alpha_dot = Gamma * Y^T * tau_err
    YTtau = np.zeros(3)
    for i in range(3):
        Si_wdot = S[:, :, i] @ wdot
...
def demo_adapt_partial_target_LQR(J_fixed = np.diag([20.0, 25.0, 15.0]),    
    ...
    J_true = R_bt @ (Jt_diag_true) @ R_bt.T + J_fixed
    Jt_true = J_true - J_fixed
    J_nom = J_true + (S1 + S2 + S3)

    # ----- LQR design on small-angle model -----
    # x = [delta_theta; omega], delta_theta ≈ 2*q_vec
    A = np.block([
    ...
    R = r_torque_weight * np.eye(3)

    K, _, _ = lqr(A, B, Q, R)  # K: (3,6), u = -K x
    ...
    # First run: "NN" init
    sol = solve_ivp(
        fun=lambda t, x: dyn_partial_adapt_LQR(t, x, J_fixed, S, J_true, K, Gamma),
        t_span=(0.0, tf),
        y0=x0,
        rtol=1e-12,
        atol=1e-12,
    ...
    plt.legend([r'$\delta\theta_x$', r'$\delta\theta_y$', r'$\delta\theta_z$'])
    plt.grid(True)
    plt.title('LQR Attitude error (' + initialization + ' Initialization)')
...
if __name__ == "__main__":
    ...
    J_hat_NN, J_true, Jt_true   = demo_adapt_partial_target(J_fixed = J_fixed, Jt_diag_true=Jt_diag_true, Jt_est=Jt_est,initialization="NN",Kom = 10000,Kr = 3000,Gamma_scale=15.0,R_bt=R_bt)
    J_hat_rand, J_true, Jt_true = demo_adapt_partial_target(J_fixed = J_fixed, Jt_diag_true=Jt_diag_true, Jt_est=Jt_est_rand, initialization="random",Gamma_scale=15.0,Kom = 10000,Kr = 3000,R_bt=R_bt)


    J_hat_LQR, J_true, Jt_true  = demo_adapt_partial_target_LQR(J_fixed = J_fixed, Jt_diag_true=Jt_diag_true, Jt_est=Jt_est,initialization="NN",R_bt=R_bt,Gamma_scale=100000)
    J_hat_rand_LQR, J_true, Jt_true = demo_adapt_partial_target_LQR(J_fixed = J_fixed, Jt_diag_true=Jt_diag_true, Jt_est=Jt_est_rand, initialization="random",R_bt=R_bt,Gamma_scale=10000)
    plt.show()
