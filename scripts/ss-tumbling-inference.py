...
def main(plot=True):
    ...
    def validate(model,trainer,infer_num=1):
        ...
        print("eigs_unscaled:", np.array_str(np.linalg.eigvalsh(Itrue_unscaled), precision=6))
        ...
        return np.diag(np.linalg.eigvalsh(I_pred_np)),np.diag(np.linalg.eigvalsh(Itrue_unscaled)), quat_to_R_np(q[-1].cpu().numpy())
    # model + trainer
    ...
    return I_pred_lstm, I_pred_mamba, I_pred_transformer, I_pred_tcn, I_true, R, qf, wf
def demo_adapt_partial_target(J_fixed = np.diag([20.0, 25.0, 15.0]),    
    ...
    if args.save:
        plt.savefig(log_dir+"/attitude_rates_"+initialization+".png")
    ...
    if args.save:
        plt.savefig(log_dir+"/target_inertia_"+initialization+".png")
    ...
    if args.save:
        plt.savefig(log_dir+"/total_inertia_"+initialization+".png")
    ...
    if args.save:
        plt.savefig(log_dir+"/alpha_vs_bestfit_"+initialization+".png")
...
if __name__ == "__main__":
    ...
    print("True target inertia       Jt_true:\n", np.array_str(np.array(I_true), precision=4, suppress_small=True))
    print("Estimated target inertia Jt_est (from random generator):\n", np.array_str(np.array(Jt_est_rand), precision=4, suppress_small=True))
    print("Estimated target inertia Jt_est (from LSTM model):\n", np.array_str(np.array(I_pred_lstm), precision=4, suppress_small=True))
    print("Estimated target inertia Jt_est (from Mamba model):\n", np.array_str(np.array(I_pred_mamba), precision=4, suppress_small=True))
